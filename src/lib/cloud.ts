/**
 * cloud.ts - Capa de nube (Supabase) para CyC Clothes.
 ...
 * propias de esta app (clot_*). Prefijo de datos: CLOT.
 */
...
export async function asegurarCuentaSeguraColab(usuario: string, password: string, codigo: string) {
  ...
  if (!sess) {
    let ok = false;
    try { ok = await rpc('clot_verificar_colab', { p_codigo: codigo, p_usuario: usuario, p_pass: password }, false); }
    catch (e) { ok = false; }
  ...
  try { await rpc('clot_unir_colab', { p_codigo: codigo, p_usuario: usuario }); }
...
// -- Sync de datos del local (tabla clot_backups, RLS por membresia) --
export async function cloudLoad(codigo: string): Promise<CloudData | null> {
  const bearer = (await authToken()) || SB_KEY;
  try {
    const res = await fetch(
      `${SB_URL}/rest/v1/clot_backups?tenant_id=eq.${encodeURIComponent(codigo)}&select=datos&limit=1`,
...
export async function cloudSave(codigo: string, datos: CloudData): Promise<boolean> {
  const bearer = (await authToken()) || SB_KEY;
  try {
    const res = await fetch(`${SB_URL}/rest/v1/clot_backups`, {
      method: 'POST',
      headers: {
...
// -- Historial / rollback (clot_backups_hist) --
export async function histListar(codigo: string): Promise<any[]> {
  try { const d = await rpc('clot_hist_listar', { p_codigo: codigo }); return Array.isArray(d) ? d : []; }
  catch (e) { return []; }
}
export async function histRestaurar(codigo: string, id: number): Promise<CloudData | null> {
  try { const d = await rpc('clot_hist_restaurar', { p_codigo: codigo, p_id: id }); return (d && d.ok) ? d.datos as CloudData : null; }
  catch (e) { return null; }
}
export async function histSnapshot(codigo: string): Promise<boolean> {
  try { const d = await rpc('clot_hist_snapshot', { p_codigo: codigo }); return !!(d && d.ok); }
  catch (e) { return false; }
}

// -- Publico (catalogo) y alta de pedido/resena --
export async function clotPublica(codigo: string): Promise<CloudData | null> {
  try { return await rpc('clot_publica', { p_codigo: codigo }, false) as CloudData; }
  catch (e) { return null; }
}

export async function clotAgregarPedido(codigo: string, pedido: any): Promise<void> {
  try { await rpc('clot_agregar_pedido', { p_codigo: codigo, p_pedido: pedido }, false); }
  catch (e) { /* noop */ }
}

export async function clotAgregarResena(codigo: string, comentario: any): Promise<void> {
